...
>
> Last updated: 2026-04-25 (T-14h to demo) — **Three demo-breaking bugs found by frame extraction, fixed, and pinned with regression tests. 65 green tests.** Deadline: 2026-04-26 11:00am EDT.
>
> **The honest delta from the last "everything is fine" claim.** Three bugs survived the previous test pass because the tests checked URL strings, not what Cloudinary actually rendered. Frame extraction caught them:
>
> 1. **Captions placed mid-frame, not lower-third.** The `l_text:` overlays in both the cached lighthouse bake AND the editor's per-clip captions had `g_south,y_120` glued to the text declaration segment. Cloudinary silently centers the caption when positioning sits in the opener instead of the `fl_layer_apply` closer. Captions covered the keeper's chest and face for the full 6 seconds of every captioned beat. **Fixed in `cloudinary.py:_static_caption_overlay` and `cloudinary.py:_caption_overlay`; URL pinned in `cached.py:LIGHTHOUSE_SHIP_FINAL_URL`. Pinned by 3 regression tests** that diff the URL structure (positioning must live in `/fl_layer_apply,g_south,y_120`, never inline with `l_text:`).
>
...
> - **65 green tests** (was 58, +7 regression tests covering the bugs above). Tests now check URL *structure* (segment-by-segment), not just URL *substrings* — the previous tests would have green-lit all three of the bugs above.
...
> - **Real-mode video pipeline (Veo 3): end-to-end verified at production quality.** Default model is now `veo-3.0-generate-001` (Veo 3 GA). Veo now receives the full cinematic prompt (image + motion + voiceLine), not just the motion fragment. `generateAudio: true` + `resolution: 1080p` are passed through. Native synchronized audio (dialogue, SFX, ambient) lands in the rendered clip — confirmed via `ffprobe` on a single-beat smoke (stereo AAC 48kHz, mean -31 dB / max -10 dB, ~6s) and the full 7-beat bake (1920×1080, 48s, AAC stereo).
