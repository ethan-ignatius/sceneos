...
> 5. **Cross-beat amnesia — REAL root cause.** The rich `_earlier_beats_block` was being passed via Gemini's `system_instruction` field, but Gemini 2.5 weights the user channel much heavier than the system instruction for in-context recall. On a fresh beat the SEED user message was just the generic `"My idea: <master>. Ask me your first question..."` — the model anchored to that and started fresh, ignoring the established canon sitting in the system prompt. New `_prior_beats_recap` injects the established facts (full beatFacts + verbatim user turns from each prior beat) directly into the seed user turn with an explicit "build on this, do not start fresh" instruction. Cross-beat continuity now arrives via the channel the model actually attends to.
>
